...
RegisterServerEvent('esx_nightclub:buyTicket')
AddEventHandler('esx_nightclub:buyTicket', function(enter, checkMoney)
local _source = source
local xPlayer = ESX.GetPlayerFromId(_source)
local enter = Config.Enter.cost.enter                                                                                                                                                                
local wob = xPlayer.getInventoryItem('ticketwob').count
local societyAccount = nil
...
TriggerEvent('esx_addonaccount:getSharedAccount', 'society_nightclub', function(account)
	...
	if (xPlayer.getMoney() >= enter) and not (wob >= 1) then
        
		xPlayer.removeMoney(enter)
		societyAccount.addMoney(enter)
        TriggerClientEvent('esx:showNotification', _source, _U('have_fun'))

	elseif (xPlayer.getAccount('bank').money >= enter) and not (wob >= 1) then
			

			xPlayer.removeAccountMoney('bank', enter)
			societyAccount.addMoney(enter)
			TriggerClientEvent('esx:showNotification', _source, _U('have_fun'))
	
    elseif (xPlayer.getMoney() < enter) and not (wob >= 1) then
        ...
        TriggerClientEvent('esx:showNotification', _source, _U('have_fun_vip'))
